...
Nothing illustrates the tight link between New York City and its history of gun violence quite like the near reproduction of the city map simply by plotting all shooting incidents according to their geographical coordinates. See @fig-NYC in annex for a reference map of NYC.
...
clean_data |>
  ...
  ggplot(aes(x=year, y=`Number of shootings`, group=PRECINCT, color=BORO)) +
    geom_line() + 
    geom_point() +
    geom_smooth(aes(group = 1), formula = y ~ x) +
    scale_x_continuous(breaks=c(seq(2006,2022,by=2))) +
...
[GitHub repository](https://github.com/briandekorodi/NewYorkShootings)

### Data import work :

The main data set, which is an historic of all shooting incidents that happened in NYC between 2006 and 2022, is from [the US government open data website](https://catalog.data.gov/dataset/nypd-shooting-incident-data-historic/resource/c564b578-fd8a-4005-8365-34150d306cc4).

To display the shooting incidents on the map of NYC, it was joined with a [shape file of NYC precincts](https://data.cityofnewyork.us/Public-Safety/Police-Precincts/78dh-3ptz), using the common column "precinct".

To produce the map of the Displacement Risk Index in NYC (2022), data from the [Displacement Risk Map of NYC](https://equitableexplorer.planning.nyc.gov/map/drm/nta) was used.

![*Map of New York City*](NYC_Map.png){#fig-NYC}
