...
```{r}
# add spatial coordinates to the previous dataset
...
ggplot() +
  geom_sf(data = shootings_precincts_per_periods, aes(geometry=geometry, fill=`Average number of shootings`)) +
...
```{r}
nyc_nta <- st_read("nynta2010_23d/nynta2010.shp")
```

```{r}
nyc_nta_gen <- NTA_data |> left_join(nyc_nta, join_by(NTACode)) 
```

```{r}

ggplot() +
  geom_sf(data = nyc_nta_gen, aes(geometry=geometry, fill=`DisplacementRiskIndex`)) +
  labs(title = "Displacement risk index 2022 (Gentrification potential)")

```

This information seems to point to the fact that the two most violent zones in Brooklyn and the Bronx, have not been gentrified over the time span of our study, because they are still at high risk of gentrification today.
...
We see that no matter which borough, a vast majority (almost 80%) of shootings happen in homes.

### Neighbourhood gentrification

```{r}
here::i_am("NewYorkShootings.Rproj")
NTA_data <- vroom("DRI_Subindices_Indicators.csv")
```
