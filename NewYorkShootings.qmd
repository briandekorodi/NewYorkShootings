...
```{r}
ny <- vroom("data/NYPD_Shooting_Incident_Data__Historic.csv")
```

```{r}
ny |> ggplot(aes(x=X_COORD_CD, y=Y_COORD_CD, color=BORO))+geom_point()
```

```{r}
ny |> ggplot(aes(x=Longitude, y=Latitude, color=BORO))+geom_point()
```
