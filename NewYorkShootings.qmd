...
Several columns of the dataset contain some missing values for which it is NA and some for which it is "(null)". We replace all the "(null)" values in all columns by NA.
...
```{r}
clean_data[["VIC_AGE_GROUP"]] |> typeof()
```
...
clean_data <- clean_data |>
  mutate(VIC_AGE_GROUP = ifelse(VIC_AGE_GROUP %in% c("1022", "UNKNOWN"), NA, VIC_AGE_GROUP))
...
```{r}
clean_data <- clean_data |>
  mutate(PERP_AGE_GROUP = ifelse(PERP_AGE_GROUP %in% c("1020", "940", "224", "UNKNOWN"), NA, PERP_AGE_GROUP))
...
```{r}
clean_data |> distinct(LOC_OF_OCCUR_DESC)
```
...
```{r}
clean_data <- clean_data |>
  mutate(LOCATION_DESC = recode(LOCATION_DESC, "MULTI DWELL - PUBLIC HOUS" = "MULTI DWELL - PUBLIC HOUSE", "SOCIAL CLUB/POLICY LOCATI" = "SOCIAL CLUB/POLICY LOCATION"),
         LOCATION_DESC = replace(LOCATION_DESC, LOCATION_DESC == "NONE", NA))

clean_data |> distinct(LOCATION_DESC)                         
```

```{r}
clean_data |> distinct(STATISTICAL_MURDER_FLAG)
```
...
```{r}
clean_data |> distinct(VIC_RACE)
```

```{r}
clean_data |> distinct(PERP_RACE)
```
